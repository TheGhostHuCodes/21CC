/** \file process_dir.h
 *  \brief This is a file declares functions that work with filesystem entries.
 */
struct filestruct;
 ...
 * and for each directory run \c dir_action and recurse into the directory.
 * Note that this makes the traversal depth first.
 *
 * Your functions will take in a \c filestruct, qv. Note that there is an \c
 * error element, which you can set to one to indicate an error.
 *
 ...
 * call for every non-directory file.
