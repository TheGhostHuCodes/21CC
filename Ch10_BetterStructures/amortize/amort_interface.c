/** \file amort_interface.c
 *  \brief This file does input validation and selects reasonable default
 *  values for the amortization method.
 */
...
amortization_s amortize_prep(amortization_s in) {
    Stopif(!in.amount || in.amount < 0 || in.rate < 0 || in.months < 0
               || in.years < 0 || in.selloff_month < 0 || in.selloff_year < 0,
    ...
    out.interest
        = amortize(in.amount, out.rate, in.inflation, months, selloff_month,
