/** \file amortize.c
 *  \brief This is a file that does math.
 */
#include "amortize.h"
#include <math.h>
#include <stdio.h>
...
double amortize(double amt, double rate, double inflation, int months,
    ...
    *monthly_payment
        = amt * mrate / (1 - pow(1 + mrate, -months)) + extra_payoff;
    if (verbose) {
        printf("Your total monthly payment: %g\n\n", *monthly_payment);
    }
    int end_month
        = (selloff_month && selloff_month < months) ? selloff_month : months;
    if (verbose) {
    ...
    for (m = 0; m < end_month && amt > 0; ++m) {
        ...
        double deflator = pow(1 + inflation / 100, -m / (double)12);
        *interest_pv += interest_payment * deflator;
        total_interest += interest_payment;
        if (verbose) {
            printf("%i/%i\t%7.2f\t\t%7.2f\t| %7.2f\t %7.2f\t%7.2f\n", m / 12,
                   m - 12 * (m / 12) + 1, principal_payment, interest_payment,
                   principal_payment * deflator, interest_payment * deflator,
                   principal_payment / (principal_payment + interest_payment)
                       * 100);
        }
    }
    *duration = m / (double)12;
