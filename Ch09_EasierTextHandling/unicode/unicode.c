...
char* localstring_to_utf8(char* instring) {
    ...
    free(instring);
    Stopif(!g_utf8_validate(out, -1, NULL), free(out);
           return NULL, "Trouble: I couldn't convert your file to a valid "
                        "UTF-8 string.");
    return out;
}

int main(int argc, char** argv) {
    Stopif(argc == 1, return 1,
    ...
    fprintf(out, "This document has %li characters.<br>",
            g_utf8_strlen(ucs, -1));
    fprintf(out, "Its Unicode encoding requires %zu bytes.<br>", strlen(ucs));
    ...
    for (int i = 0; i < spaced->length; ++i, ++(spaced->elements)) {
        ...
        if (g_unichar_isalpha(c)) {
            fprintf(out, " (a letter)");
        }
        if (g_unichar_isdigit(c)) {
            fprintf(out, " (a digit)");
        }
        if (g_unichar_iswide(c)) {
            fprintf(out, " (wide, CJK)");
        }
        fprintf(out, "<br>");
    }
    fclose(out);
