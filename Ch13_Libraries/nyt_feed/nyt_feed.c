...
/** \mainpage
 * The front page of the Grey Lady's website is as gaudy as can be,
 ...
 *
 * This program reads in the NYT Headlines RSS fead, and writes a simple list
 * in plain HTML. You can then click through to the headline that mostly piques
 * your attention.
 *
 ...
 * LDLIBS = `curl-config --libs` -lxml2 - lpthread CC=clang
 *
 * nyt_feed: \endcode
 *
 ...
 * length of the \c urls list or the \c titles list is \c NULL, this will
 * crash.
 */
 ...
 * nodes matching the XPath for the title elements and all nodes matching the
 * XPath for the links. Then, it will write these to the outfile.
 *
...
int parse(const char* infile) {
    ...
    Stopif(!context, return -2, "Error: unalbe to create new XPath context\n");
