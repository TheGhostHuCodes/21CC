...
int regex_match_base(regex_fn_s in) {
    ...
    int matchcount = 0;
    if (in.substrings) {
        matchcount = count_parens(in.regex);
    }
    regmatch_t result[matchcount + 1];
    int compiled_ok = !regcomp(&re, in.regex,
                               REG_EXTENDED + (in.use_case ? 0 : REG_ICASE)
                                   + (in.substrings ? 0 : REG_NOSUB));
    Stopif(!compiled_ok, return -3,
           "This regular expression didn't compile: \"%s\"", in.regex);
    ...
    if (in.substrings) {
        *in.substrings = malloc(sizeof(char*) * matchcount);
        ...
        // Match at index zero is the whole string, ignore it.
        ...
        in.string += result[0].rm_eo; // End of the whole match.
    }
    regfree(&re);
    return matchcount;
}
...
int main() {
    ...
    int match_ct
        = regex_match("Hedonism by the alps, savory foods at every meal.",
